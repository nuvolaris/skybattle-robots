...
function main(args){
    ...
    switch(args.event) {
        case "idle":
            ...
            if(isStart(data)){
               ...
               if(isCyan(args.x)){          
                    ...
                    actions.push({...tankAndTurretRotationTogether(rotation), "yell": rotation.yell})

                    actions.push({...shoot()})

                    rotation = rotate(args.tank_angle, 45) 

                    actions.push({...tankRotation(rotation), "yell": rotation.yell})
                    ...
                    rotation = rotate(args.tank_angle, 225) 

                    actions.push({...tankRotation(rotation), "yell": rotation.yell})  

                } 

                let move_action = move(data.move_direction, 166)
                ...
                if(data.spotted === true){
                    ...
                    if(hasToShoot(args.angle, data.last_enemy_spot.angle, data.last_enemy_spot.distance))
                        actions.push({...shoot()})                                                                     
                    actions.push({...shoot()})
                    data.spotted = false
                }else if(data.last_enemy_spot != undefined && (Date.now() - data.last_spotted_time) < 1000){
                    //let my_next_position = myNextPosition(args.x, args.y, args.tank_angle, MOVE_AMOUNT,  data.move_direction)
                    //let enemy_next_position = enemyNextPosition(data.last_enemy_spot.x, data.last_enemy_spot.y, data.last_spotted_time, data.enemy_spot_history)
                    let angle = angleFromCoords(args.x, args.y, data.last_enemy_spot.x,  data.last_enemy_spot.y)
                    //let angle = angleFromCoords(args.x, args.y, data.last_enemy_spot.x, data.last_enemy_spot.y )
        ...
        case "enemy-spot":
            ...
            actions.push({...tankRotation(rotation)})
            if(hasToShoot(args.angle, args.enemy_spot.angle, args.enemy_spot.distance))
                actions.push({...shoot()})
